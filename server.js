...
function getMockup(e, positionInfo, isPlayerEntity = false) {
  return {
    // Core identification fields - MUST be first for WASM parsing
    type: 2,  // Entity type: 2 = regular entity
    index: e.index,
    flags: isPlayerEntity ? 2 : 0,
    player: isPlayerEntity ? 1 : 0,  // 1 for player entity, 0 for others
...
for (let k in Class) {
  try {
    ...
    // Add the new data to the thing - MARK FIRST ENTITY AS PLAYER
    mockupData.push(getMockup(temptank, type.mockup.position, mockupData.length === 0));
  ...
  return {
    ...
    connect: (() => {
      ...
      function incoming(message, socket) {
        ...
        console.log('[SERVER] Received packet:', rawBytes);
        
        // Decode message with fallback for raw WASM bytes
        let m = protocol.decode(message);
        
        // If normal fasttalk decode fails, try with 0xFF prefix wrapper
        if (m === -1 || m === null) {
          console.log('[INFO] Fasttalk decode failed, trying with 0xFF prefix wrapper...');
          
          // Wrap with 0xFF header and retry
          const wrapped = new Uint8Array(rawBytes.length + 1);
          wrapped[0] = 0xFF;
          wrapped.set(rawBytes, 1);
          
          try {
            m = protocol.decode(wrapped.buffer);
            console.log('[INFO] Wrapped decode result:', m);
          } catch (e) {
            console.log('[INFO] Wrapped decode also failed:', e.message);
            // For WASM clients, perhaps handle raw
            console.log('[INFO] Treating as raw WASM packet, ignoring');
            return 0; // Don't kick
          }
          
          if (m === -1 || m === null || (Array.isArray(m) && m.length === 0)) {
            console.log('[INFO] Treating raw bytes as message array');
            m = Array.from(rawBytes);
          }
        }
        
        // Debug logging
        console.log('[INFO] Incoming message decoded:', m, 'type:', typeof m);
        
        // Make sure it looks legit - should be an array now
        if (!Array.isArray(m)) {
          console.error("[ERROR] Decoded message is not an array:", m);
          socket.kick("Invalid message format.");
          return 1;
        }
        ...
        // Ignore empty arrays (they're from raw packet wrapping with 0xFF prefix)
        if (m.length === 0) {
          console.log("[INFO] Ignoring empty command array (raw packet wrapper)");
          return 0;
        }
        
        // Log decoded message for debugging
        if (m.length > 0 && typeof m[0] !== 'string') {
          // Decode raw packet: map first byte to command name
          let commandByte = m[0];
          let command;
          if (commandByte === 0) command = "spawn";
          else if (commandByte === 1) command = "ping";
          else command = "unknown_" + commandByte;
          let data = m.slice(1);
          console.log("[INFO] Decoded raw packet:", `"${command}"`, ...data);
        }
        // Remember who we are
        let player = socket.player;
        // Handle the request
        switch (m.shift()) {
          ...
          case 1:
            {
                        console.log("[DEBUG] Handling command:", m[0]);
              // key verification
              if (m.length > 1) {
              ...
              // Skip invalid 'w' message - WASM client doesn't handle it
              // Send verification confirmation with valid message type instead
              console.log("[SERVER] Player verified, skipping invalid 'w' message");
              if (m.length === 1) {
                let key = m[0];
                socket.key = key;
...
case 0:
  {
    // Validate spawn request first
    if (!socket.status.deceased) {
      socket.kick("Trying to spawn while already alive.");
      return 1;
    }
    // Get data - handle raw bytes from WASM client
    let needsRoom = m.shift();  // First element after shift is needsRoom
    let name;
    if (m.length > 0) {
      if (typeof m[0] === 'string') {
        // Normal case: name is already a string
        name = m[0].replace(c.BANNED_CHARACTERS_REGEX, "");
      } else {
        // Raw bytes case: decode remaining bytes as UTF-8 string
        name = String.fromCharCode(...m).replace(c.BANNED_CHARACTERS_REGEX, "");
      }
    } else {
      name = "";  // Fallback if no name provided
    }
    // Verify needsRoom
    if (typeof needsRoom !== "number" || (needsRoom !== 0 && needsRoom !== 1)) {
    ...
    if (encodeURI(name).split(/%..|./).length > 48) {
      ...
      return 1;
              ...
              // Send room state FIRST (before entities)
              if (!needsRoom) {
                console.log("[SERVER] Sending ROOM message - width:", room.width, "height:", room.height);
                socket.talk(
                  "R",
                  room.width,
                  room.height,
                ...
                console.log("[SERVER] ROOM message sent");
              }
              ...
              // DISABLED: Send any additional messages to avoid WASM crashes
              // 2022 WASM is incompatible with 2020 server message format
              
              console.log("[SERVER] Skipping JOIN and welcome messages - waiting for WASM update requests");
              
              // Start the update rhythm immediately (UPDATE messages are what WASM needs)
              socket.update(0);
              // Log it
              util.log(
                "[INFO] " +
          ...
          case "d":
            {
              ...
              // Flush any buffered messages for WASM compatibility
              // The WASM protocol requires strict message ordering - only one update at a time
              if (socket.messageBuffer && socket.messageBuffer.length > 0) {
                let nextMessage = socket.messageBuffer.shift();
                socket.send(protocol.encode(nextMessage), { binary: true });
              } else {
                socket.isBuffering = false;
              }
          ...
          default:
            ...
            console.warn("Unknown packet command:", m.length > 0 ? m[0] : "empty");
        }
      }
      ...
      const spawn = (() => {
        ...
        return (socket, name) => {
          ...
          // Queue spawn messages with slight delay to avoid flooding WASM
          // These will be buffered and sent one at a time
          setTimeout(() => {
      ...
      return (socket, req) => {
        ...
        // Initialize message buffer for WASM compatibility
        socket.messageBuffer = [];
        socket.isBuffering = false;
        socket.maxBufferSize = 1; // Only allow 1 queued message at a time for WASM
        socket.minMessageInterval = 16; // Minimum milliseconds between messages (for 60fps target)
        // Set up the viewer
        socket.makeView = () => {
        ...
        socket.talk = (...message) => {
          console.log('[SERVER] Sending packet:', message);
          if (socket.readyState === socket.OPEN) {
            // For WASM compatibility, implement message buffering to prevent message flooding
            // Only queue if there's already a message being processed
            if (socket.isBuffering && socket.messageBuffer.length >= socket.maxBufferSize) {
              // Drop the oldest buffered message if queue is full (keep only the latest)
              if (socket.messageBuffer.length > 0) {
                socket.messageBuffer.shift();
              }
            }
            
            if (socket.isBuffering) {
              socket.messageBuffer.push(message);
            } else {
              socket.send(protocol.encode(message), { binary: true });
              socket.isBuffering = true;
            }
          }
        };
        socket.lastWords = (...message) => {
          if (socket.readyState === socket.OPEN) {
            // For WASM clients, send raw byte instead of encoded
            if (message.length === 1 && message[0] === "K") {
              socket.send(new Uint8Array([75]), { binary: true }, () =>
