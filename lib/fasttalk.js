  ...
  try {
    ...
    while (true) {
      if (index >= data.length) {
        console.error('Header decoding overflow')
        try {
          return decodeLegacy(packet)
        } catch (e) {
          return -1
    ...
    if ((typeCode & 0b1100) === 0b1100) {
      ...
      if (typeCode === 0b1110) {
        if (index >= data.length) {
          console.error('Header decoding overflow on repeat')
          try {
            return decodeLegacy(packet)
          } catch (e) {
            return -1
...
let decodeLegacy = (() => {
  ...
  return raw => { try {
    ...
    while (offset < str.length) {
      ...
      default:
        offset = str.length
        // Don't throw, just return -1 to indicate decode error
        return -1
      }
    }
    return output
...
if (typeof window !== 'undefined' && typeof module === 'undefined') {
  window.fasttalk = { encode: encode, decode: decode };
}
